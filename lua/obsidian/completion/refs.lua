...
M.can_complete = function(request)
  ...
  if vim.startswith(input, "[[") then
    ...
    return true, search, cursor_col - #input, cursor_col + insert_end_offset, M.RefType.Wiki
  elseif vim.startswith(input, "[") then
    ...
    return true, search, cursor_col - #input, cursor_col + insert_end_offset, M.RefType.Markdown
  else
    return false
  end
end
