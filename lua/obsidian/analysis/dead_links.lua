  ...
  for line_idx, line in ipairs(lines) do
    for _, ref in ipairs(parse_refs.extract(line)) do
      ...
      if
        ...
        and not api.is_attachment_path(location)
      then
        local target = resolve_target_path(source_path, location, reference_lookup, absolute_lookup)
        if target == nil then
          local start_col = ref.range.start_col + (ref.embed and 1 or 0)
          local link = ref.embed and ref.raw:sub(2) or ref.raw
          local key =
            table.concat({ tostring(line_idx), tostring(start_col), tostring(ref.range.end_col), location }, ":")
