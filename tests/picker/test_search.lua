local M = require "obsidian.picker.search"
...
local documents = {
  M.from_lines("/vault/meetings/work.md", {
    "---",
    "status: Draft",
    "duration: 3",
    "aliases: []",
  ...
  M.from_lines("/vault/personal/meetup.md", {
    "---",
    "status: Published",
    "rating:",
    "---",
  ...
  M.from_lines("/vault/archive.md", { "old work notes" }, { root = "/vault" }),
}

local function paths(query)
  return vim.tbl_map(function(result)
    return result.text
  end, M.search(documents, query))
end

T["parser"] = new_set()

T["parser"]["uses implicit AND before OR and supports negated groups"] = function()
  local ast = assert(M.parse "meeting work OR meetup personal")
  eq("or", ast.kind)
...
T["parser"]["reports incomplete delimiters unless requested otherwise"] = function()
  local _, quote_error = M.parse '"unfinished'
  local _, group_error = M.parse "meeting (work OR personal"
  eq(true, quote_error ~= nil)
  eq(true, group_error ~= nil)
  eq("and", assert(M.parse("meeting (work OR personal", { allow_incomplete = true })).kind)
end
...
T["results are ranked by match quality and then by path"] = function()
  ...
  local results = M.search(documents, "HappyCat")
  eq(11, results[1].lnum)
end

T["index_async indexes notes and canvases before returning"] = function()
  local Path = require "obsidian.path"
  local dir = Path.temp { suffix = "-picker-search" }
  ...
  local indexed
  M.index_async(dir, {}, function(result)
    indexed = result
  end)
