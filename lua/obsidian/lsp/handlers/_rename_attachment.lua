      ...
      vim.schedule(function()
        ...
        for _, match in ipairs(processed_lines) do
          ...
          if not seen_lines[line_key] then
            ...
            for _, ref in ipairs(search.find_refs(line_text, { exclude = { "BlockID" } })) do
              local ref_start, ref_end, link_type = unpack(ref)
              local ref_text = line_text:sub(ref_start, ref_end)
              local location, name = util.parse_link(ref_text, { link_type = link_type })
              if
                location
