...
---Scan a single note file for mentions of self_symbols, appending PickerEntry items to entries.
---@param path string
---@param self_symbols obsidian.LinkSuggestionSymbol[]
---@param entries obsidian.PickerEntry[]
local function scan_note(path, self_symbols, entries)
  local norm_path = vim.fs.normalize(path)

  -- Fast pre-filter: check if any symbol text appears in the raw file before a full parse.
      ...
      for _, suggestion in ipairs(line_suggestions) do
        for _, candidate in ipairs(suggestion.candidates) do
          ...
          entries[#entries + 1] = {
            ...
            lnum = lnum,
            col = col,
            text = string.format("%s:%d:%d  %s → %s", short_path, lnum, col, suggestion.text, candidate.new_text),
...
return function()
  ...
  -- Filter link_suggestion.symbols to only those resolving to the current note's path.
  ...
  for path, _ in pairs(cached_rows) do
    if vim.fs.normalize(path) ~= vim.fs.normalize(current_path) then
      scan_note(path, self_symbols, entries)
    end
  end
  ...
  picker.select(entries, {
    ...
    for _, choice in ipairs(choices) do
      ...
      if data then
        ...
        -- If the file was not previously loaded, unload it to avoid polluting the buffer list.
