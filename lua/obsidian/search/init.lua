...
---@return string|? block
local function parse_ref_target(ref_text, ref_type)
  ...
  local block
  link_location, block = util.strip_block_links(link_location)

  local anchor
  link_location, anchor = util.strip_anchor_links(link_location)

  return link_location, anchor, block
end
...
---@return boolean
local function submatch_in_ref(submatches, ref_start, ref_end)
...
---@param opts { search: obsidian.SearchOpts|?, anchor: string|?, block: string|?, dir: string|obsidian.Path|?, refs: string[]|?, loose_mode: boolean|? }|?
---@return vim.SystemObj handle
M.find_backlinks_async = function(note, callback, opts)
  ...
  local _on_match = function(match)
    ...
    for _, ref in ipairs(M.find_refs(line_text)) do
      ...
      if submatch_in_ref(match.submatches, ref_start, ref_end) then
        ...
        local link_location, matched_anchor, matched_block = parse_ref_target(ref_text, ref_type)
        if link_location then
          ...
          if block and include then
            if not matched_block or util.standardize_block(matched_block) ~= block then
              include = false
            end
          end
          if include then
  ...
  return M.search_async(
    ...
    vim.tbl_extend("force", opts.search or {}, { fixed_strings = true, ignore_case = true }),
    _on_match,
    function()
      callback(results)
    end
...
---@param note obsidian.Note
---@param opts { search: obsidian.SearchOpts, anchor: string, block: string, timeout: integer, dir: string|obsidian.Path, refs: string[]|?, loose_mode: boolean|? }?
---@return obsidian.BacklinkMatch[] matches always returns a list (empty on timeout)
M.find_backlinks = function(note, opts)
  opts = opts or {}
  opts.timeout = opts.timeout or 1000
  local result = async.block_on(function(cb)
    return M.find_backlinks_async(note, cb, {
      search = opts.search,
      anchor = opts.anchor,
      block = opts.block,
      dir = opts.dir,
      refs = opts.refs,
      loose_mode = opts.loose_mode,
    })
  end, opts.timeout)
  return result or {}
end
