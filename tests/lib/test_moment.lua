...
T["format YY"] = function()
  eq(M.format(date, "YY"), "09")
end

T["format escape brackets"] = function()
  eq(M.format(date, "[day]"), "day") -- Single bracket
  eq(M.format(date, "[day] YY [YY]"), "day 09 YY") -- Double bracket
  eq(M.format(date, "[YY"), "[09") -- Un-ended bracket
  eq(M.format(date, "[[YY]]"), "[YY]") -- Double nested brackets
  eq(M.format(date, "[[]"), "[") -- Escape open bracket
  eq(M.format(date, "[Last]"), "Last") -- Escape open bracket
  eq(M.format(date, "[L] L"), "L 02/14/2009") -- localized tokens with escaped localized tokens
  eq(M.format(date, "[LLL] LLL"), "LLL February 14, 2009 3:25 PM") -- localized tokens with escaped localized tokens (recursion)
  eq(M.format(date, "[L LL LLL LLLL aLa]"), "L LL LLL LLLL aLa") -- localized tokens with escaped localized tokens
  eq(M.format(date, "YYYY[\n]DD[\n]"), "2009\n14\n") -- Newlines
end
...
-- TODO: mock api
-- T["Y token"] = function()
--   eq(moment("2010-01-01", "YYYY-MM-DD", true).format "Y", "2010", "format 2010 with Y")
--   eq(moment("-123-01-01", "Y-MM-DD", true).format "Y", "-0123", "format -123 with Y")
--   eq(moment("12345-01-01", "Y-MM-DD", true).format "Y", "+12345", "format 12345 with Y")
--   eq(moment("0-01-01", "Y-MM-DD", true).format "Y", "0000", "format 0 with Y")
--   eq(moment("1-01-01", "Y-MM-DD", true).format "Y", "0001", "format 1 with Y")
--   eq(moment("9999-01-01", "Y-MM-DD", true).format "Y", "9999", "format 9999 with Y")
--   eq(moment("10000-01-01", "Y-MM-DD", true).format "Y", "+10000", "format 10000 with Y")
-- end
--
